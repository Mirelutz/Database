...
$sql2 = "CREATE PROCEDURE AdAdept(
    IN p_nume VARCHAR(255), 
    IN p_prenume VARCHAR(255), 
    IN p_domiciliu VARCHAR(255), 
    IN p_iq INT, 
    IN p_imagine BLOB
)
BEGIN
    INSERT INTO infadepti (nume, prenume, domiciliu, iq, imagine) 
    VALUES (p_nume, p_prenume, p_domiciliu, p_iq, p_imagine);
END";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $iq = $_POST['iq'];
    $imagine = file_get_contents($_FILES['imagine']['tmp_name']);

    // Apelăm procedura stocată pentru a adăuga un nou adept
    $sql = 'CALL AdAdept(:nume, :prenume, :domiciliu, :iq, :imagine)';
    $stmt = $con->prepare($sql);
    $stmt->bindParam(':nume', $nume);
    $stmt->bindParam(':prenume', $prenume);
    $stmt->bindParam(':domiciliu', $domiciliu);
    $stmt->bindParam(':iq', $iq);
    $stmt->bindParam(':imagine', $imagine, PDO::PARAM_LOB); // Setăm tipul de date ca BLOB
    $stmt->execute();
...
<body>
    <div class="form-container">
        ...
        <form action="adaugare.php" method="post" enctype="multipart/form-data">
            ...
            <input type="file" name="imagine" placeholder="Imagine" required>
